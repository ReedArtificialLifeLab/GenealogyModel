...
class Genealogy:
    ...
    def fill_next_generation(self):
        ...
        fits = [ self.fitnesses[i] * distribution[i] for i in range(self.traits_expansion) ]
        print(fits)
        fits_total = sum(fits)
        # normalized
        fits = [ f/fits_total for f in fits ]
        # create new members
        for m in range(self.size): # for each member in the new generation
            ...
            for p in range(self.parents):
                ...
                parents.append(np.random.choice(np.arange(self.traits_expansion), p=fits))
    ...
    def inheret_traits(self,parents):
        ...
        for i in range(len(parents)):
            bin_arr = ind_to_bin(i,self.tlen) # len(bin_arr) == len(self.traits)
            for j in bin_arr:
        ...
        for i in range(len(self.traits)):
            ...
            pro_trait = traits_pool[i] / len(parents)
            con_trait = (len(parents) - pro_trait) / len(parents)
            # assign trait
